...
export function PageHero({ eyebrow, title, description, crumbs = [] }: PageHeroProps) {
  return (
    <section className="surface-paper border-b border-line/70 dark:border-border">
      <div className="mx-auto max-w-[1600px] px-4 py-14 text-center sm:px-6 lg:px-10 lg:py-20">
        {crumbs.length > 0 && (
        ...
        <h1 className="display-heading mt-5 text-[32px] text-ink sm:text-[44px] lg:text-[54px] 2xl:text-[58px] dark:text-foreground">
          {title}
        </h1>

        {description && (
          <p className="mx-auto mt-5 max-w-2xl text-[15px] lg:text-[17px] 2xl:text-[20px] leading-[1.7] text-ink-soft dark:text-foreground/70">
            {description}
          </p>
        )}
      </div>
